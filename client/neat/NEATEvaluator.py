...
class NEATEvaluator:
    ...
    # Constructor
    def __init__(self, catheter_path, fitness_data, server_config, cppn_data, number_of_individuals):
        ...
        self.__initialise_server(server_config)
    ...
    def evaluate_individuals(self, genomes, config):
        ...
        with ProcessPoolExecutor(max_workers=self.number_of_workers) as executor:
            ...
            for offsets in executor.map(self.build_offsets, list_of_cppns, chunksize=2):
    ...
    # This function returns phase offsets built by a CPPN, which is received as a parameter.
    def build_offsets(self, cppn):
        ...
        for coor_mat_id in self.coordinates_and_material_id_list:
            ...
            for layer in coor_mat_id.keys():
                ...
                for coordinate in layer_data.keys():
                    ...
                    offset_value = cppn.activate(cppn_input)
    ...
    # This function generates a .vxa file of the fittest phase offset found by NEAT algorithm.
    def get_fittest_individual_file(self, cppn, experiment_id):

        offset_list = self.build_offsets(cppn)
